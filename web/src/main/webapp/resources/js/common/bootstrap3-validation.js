...
!function($) {
   ...
   $.fn.validation.defaults = {
        validRules : [
            ...
            /*暂用，仅适用于字体业务的版本号格式校验*/
            {name: 'font-version', validate: function(value) {return (!/^\d{1,7}\.\d{1}\d{1}$/.test(value));}, defaultMsg: '请正确输入版本号，如：5.00'},
            {name: 'separator_semicolon', validate: function(value) {return (value.indexOf('；') > -1);}, defaultMsg: '不能使用中文\"；\"来分隔。'},

            //tqy新增
            {name: 'mobile', validate: function(value) {return !/^((\(\d{2,3}\))|(\d{3}\-)|(\d{3}))?1\d{10}$/.test(value);}, defaultMsg: '手机号码不正确。'},
            {name: 'zip', validate: function(value) {return !/^[1-9]\d{5}$/.test(value);}, defaultMsg: '邮政编码不存在'},
            {name: 'integer_number', validate: function(value) {return !/^[1-9]\d*?$/.test(value);}, defaultMsg: '必须是正整数'},
            {name: 'zero_integer_number', validate: function(value) {return !/^(([1-9]\d*?)|[0])$/.test(value);}, defaultMsg: '必须是正整数或者0'},
            {name: 'qq', validate: function(value) {return !/^[1-9]\d{4,11}$/.test(value);}, defaultMsg: 'QQ号码不正确'},
            {name: 'loginName', validate: function(value) {return !/^[\u0391-\uFFE5\w]+$/.test(value);}, defaultMsg: '允许汉字、英文字母、数字及下划线'},
            {name: 'safepass', validate: function(value) {return (/^(([A-Z]*|[a-z]*|\d*|[-_\~!@#\$%\^&\*\.\(\)\[\]\{\}<>\?\\\/\'\"]*)|.{0,5})$|\s/.test(value));}, defaultMsg: '密码由6位字母和数字组成'},
            {name: 'idcard', validate: function(value) {return !(/^(\d{6})(?!0000)[0-9]{4}((0[1-9]|1[0-2])(0[1-9]|1[0-9]|2[0-8])|(0[13-9]|1[0-2])(29|30)|(0[13578]|1[02])-31)(\d{3})(\d|[X,x])?$/.test(value));}, defaultMsg: '请输入正确的身份证号码'},
            //{name: 'xlk', validate: function(value) {return (!/^(?:\d+|\d{1,3}(?:,\d{3})+)?(?:\.\d+)?$/.test(value));}, defaultMsg: '请选择一个。'},
        ],
        reqmark:true,
